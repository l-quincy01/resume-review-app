...
export function FileUpload({
  ...
  const handleFileChange = (newFiles: File[]) => {
    ...
    if (nextFile.size > maxSize) {
      setError(
        `File exceeded the maximum size of ${(maxSize / (1024 * 1024)).toFixed(0)}MB`
      );
      return;
    }
  ...
  return (
    <div className="w-full h-[260px]" {...getRootProps()}>
      <motion.div
        onClick={handleClick}
        whileHover="animate"
        className="h-[260px] p-8 group/file block rounded-lg cursor-pointer w-full relative overflow-hidden border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50"
      >
        <input
          ref={fileInputRef}
